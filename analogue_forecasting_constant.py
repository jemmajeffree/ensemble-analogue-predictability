''' Make some analogue forecasts, using pre-calculated eofs.
    ...
    branched from analogue_forecasting2.py 9th Jan 2025. 
    Changed to forecast all lead times from the same analogue
'''
...
if __name__ == "__main__":
    ...
    for m in mask_list:
        ...
        for init_month in args.init_months:
            output_l = []
            pca_folder = (args.weight_folder_name_func(corr_data_name,init_month,0)+corr_data_name+'_'+m+'_' #0 is no lead time, probably ignored by function
                         +'_'.join(vars)
                         +'/')
            ...
            analogue_forecast_time = xr.concat([full_lib_time.shift(time=-l).sel(time=analogue.time) for l in args.analogue_lead_times],'L').assign_coords({'L':args.analogue_lead_times})
            init_forecast_time = xr.concat([full_obs_time.shift(time=-l).sel(time=init_ens.pred_time) for l in args.analogue_lead_times],'L').assign_coords({'L':args.analogue_lead_times})
            
            #print(time.time()-t0)
            forecast = (xr.concat([analogue]*args.analogue_lead_times.shape[0],'L').assign_coords({'L':args.analogue_lead_times})
            ...
            output_i.append(forecast.assign_coords({'init_month':init_month}).expand_dims('init_month'))
